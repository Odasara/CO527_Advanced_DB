...
CREATE USER 'A'@'localhost' IDENTIFIED BY 'PwA';
CREATE USER 'B'@'localhost' IDENTIFIED BY 'PwB';
CREATE USER 'C'@'localhost' IDENTIFIED BY 'PwC';
CREATE USER 'D'@'localhost' IDENTIFIED BY 'PwD';
CREATE USER 'E'@'localhost' IDENTIFIED BY 'PwE';
...
CREATE VIEW EmpForUserB AS SELECT Fname,Lname,Ssn,Bdate,Address,Sex,Super_ssn,Dno FROM EMPLOYEE;
GRANT SELECT ON EmpForUserB TO B@localhost;
CREATE VIEW DeptsForUserB AS SELECT DNAME,DNUMBER FROM DEPARTMENT;
GRANT SELECT ON DeptsForUserB TO B@localhost;
show grants for B@localhost;

GRANT SELECT,UPDATE ON WORKS_ON TO 'C'@'localhost';
CREATE VIEW EMP1USERC AS SELECT FNAME,MINIT,LNAME,SSN FROM EMPLOYEE;
GRANT SELECT ON EMP1USERC TO 'C'@'localhost';
CREATE VIEW PROJ1USERC AS SELECT PNAME,PNUMBER FROM PROJECT;
GRANT SELECT ON PROJ1USERC TO C@localhost;
...
SHOW GRANTS FOR D@localhost;

CREATE VIEW DNO3_EMPLOYEES AS SELECT * FROM EMPLOYEE WHERE DNO=3;
GRANT SELECT ON DNO3_EMPLOYEES TO E@localhost;
SHOW GRANTS FOR E@localhost;
